...
namespace Invoice.UI.Services
{
    public class PdfExportService
    {
        public void ExportSupplierJournal(string supplierName, IEnumerable<SupplierJournalEntry> entries)
        {
            ...
            // 🔹 نحسب الإجماليات العامة (باستثناء المردودات)
            var nonReturnEntries = list
                .Where(e => e.Description != "مردودات")
                .ToList();

            // ✅ إجمالي الفواتير (بدون المردودات)
            decimal totalInvoiceValue = nonReturnEntries.Sum(e => e.DebitAmount);
            ...
            // ✅ المبيعات الصافية
            decimal totalSales = totalInvoiceValue + totalTax14 - totalTax1;

            // ✅ إجمالي التحصيلات (لا تشمل المردودات)
            decimal totalReceipts = list
                    ...
                    page.Content().Column(column =>
                    {
                        ...
                        column.Item().AlignRight().Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                                ...
                                columns.RelativeColumn(2); // السداد
            ...
            "📅 التاريخ"
                        ...
                        column.Item().PaddingTop(25).BorderTop(1).BorderColor("#9CA3AF").Table(summary =>
                        {
                            summary.ColumnsDefinition(columns =>
                            {
                                ...
                                columns.RelativeColumn(1);
                                columns.RelativeColumn(1);
                                columns.RelativeColumn(1); // ✅ عمود إضافي
                                columns.RelativeColumn(1); // ✅ عمود إضافي
                            });
                            ...
                            summary.Cell().Element(CellHeaderStyle).Text("💵 إجمالي المبيعات").Bold();
                            summary.Cell().Element(CellHeaderStyle).Text("💳 إجمالي التحصيلات").Bold();
